...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    ...
    { id: "borrow", label: "Borrow & Return", icon: <ClipboardList size={20} />, roles: ["instructor"] },
  ...
  return (
    ...
    >
      ...
      <div className="h-28 flex items-center px-4 shrink-0 relative border-b border-slate-50">
        <div className={`flex items-center transition-all duration-300 w-full ${expanded ? "justify-start gap-4" : "justify-center"}`}>
          ...
          <div className={`flex flex-col transition-all duration-300 ${expanded ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 absolute"}`}>
            <h1 className="font-black text-slate-800 text-2xl tracking-tighter leading-none">CLAMS</h1>
            <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest mt-1">Lapsnadas</p>
          </div>
        </div>
      </div>

      {/* Navigation - Space-y-4 for clear separation */}
      ...
      {/* Footer: Logout moved higher with pb-20 */}
      <div className="p-4 pb-20"> 
        <button
          ...
          className={`w-full flex items-center rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-600 hover:text-white transition-all duration-300 shadow-sm ${
            expanded ? "gap-3 px-4 py-3.5 justify-start" : "justify-center py-3.5"
          }`}
        >
          <LogOut size={20} className="shrink-0" />
          <span className={`text-[11px] font-bold uppercase tracking-wider transition-all duration-300 ${expanded ? "w-auto opacity-100" : "w-0 opacity-0 absolute"}`}>
            Logout
          </span>
        </button>
      </div>
...
const NavItem: React.FC<{
  ...
  >
    ...
    <span className={`text-[11px] font-bold uppercase tracking-wide whitespace-nowrap transition-all duration-300 ${isExpanded ? "opacity-100" : "opacity-0 absolute"}`}>
      {label}
    </span>
    {isActive && (
