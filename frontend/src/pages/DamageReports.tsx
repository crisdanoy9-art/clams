...
const DamageReports: React.FC<DamageReportsProps> = ({ userRole }) => {
  ...
  const handleUpdateStatus = (
    id: string,
    nextStatus: "Pending" | "Under Repair" | "Resolved",
  ) => {
    setReports((prev) =>
      prev.map((report) =>
      ...
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.4em] mt-2 italic">
            {isAdmin
              ? "Master control for asset health"
              : "Report equipment issues to Raylle Admin"}
          </p>
        </div>
              ...
              <div className="text-right flex flex-col items-end gap-4">
                ...
                {isAdmin && report.status !== "Resolved" && (
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        handleUpdateStatus(report.id, "Under Repair")
                      }
